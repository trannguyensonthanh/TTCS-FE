...
export const rolePermissions: Record<UserRole, Permission> = {
  ...
  QUAN_LY_CSVC: {
    ...
    canEdit: [
      'YeuCauMuonPhong.TrangThaiChungID',
      'YcMuonPhongChiTiet.TrangThaiCtID',
      'YeuCauDoiPhong.TrangThaiYcDoiPID',
      'ChiTietDatPhong',
      'Phong',
      'LoaiPhong',
      'TrangThietBi',
...
export const canView = (role: UserRole, resource: string): boolean => {
  ...
  return (
    permissions.canView.includes('*') || permissions.canView.includes(resource)
  );
};
...
export const canCreate = (role: UserRole, resource: string): boolean => {
  const permissions = rolePermissions[role];
  return (
    permissions.canCreate.includes('*') ||
    permissions.canCreate.includes(resource)
  );
};
...
export const canEdit = (role: UserRole, resource: string): boolean => {
  const permissions = rolePermissions[role];
  return (
    permissions.canEdit.includes('*') || permissions.canEdit.includes(resource)
  );
};
...
export const canDelete = (role: UserRole, resource: string): boolean => {
  const permissions = rolePermissions[role];
  return (
    permissions.canDelete.includes('*') ||
    permissions.canDelete.includes(resource)
  );
};
...
export const canApprove = (role: UserRole, resource: string): boolean => {
  const permissions = rolePermissions[role];
  return (
    permissions.canApprove.includes('*') ||
    permissions.canApprove.includes(resource)
  );
};
...
export const canViewEvent = (
  ...
  if (
    ['ADMIN_HE_THONG', 'CB_TO_CHUC_SU_KIEN', 'BGH_DUYET_SK_TRUONG'].includes(
      role
    )
  ) {
    return true;
